...
//
// Game Variant 3: Shitty Mario Called "A Ten Minute Game", which is a survival game where
// it is difficult or near-impossible to survive more than ten minutes in the game. If the
// player is able to survive for more than ten minutes,(OPTIONAL ROUTE) then he may have the
// sponsor organization of the game (need sponsors) donate an agreed upon amount to the charity
// of their choice.
//
//==============================================================//
//                             About:
...
$(document).ready(function() {
    ...
    $(document).keydown(function(key) {
        ...
        switch(parseInt(key.which,10)) {
            ...
            case 37:
                ...
                if(playerId.attr("src") === mario_left_1){playerId.attr("src", mario_left_2);}
                else if(playerId.attr("src") === mario_left_2){playerId.attr("src", mario_left_1);}
                else { playerId.attr("src", mario_left_1);}
                playerId.animate({left: "-=20px"}, 10);
            ...
            case 38:
                ...
                playerId.attr("src", mario_up_1);
                playerId.animate({top: "-=20px"}, 10);
                //$('#lastKeyPress').text("Last Key: " + key.which);
                break;
            ...
            case 39:
                ...
                if(playerId.attr("src") === mario_right_1){playerId.attr("src", mario_right_2);}
                else if(playerId.attr("src") === mario_right_2){playerId.attr("src", mario_right_1);}
                else {playerId.attr("src", mario_right_1);}
                playerId.animate({left: "+=20px"}, 10);
                //$('#lastKeyPress').text("Last Key: " + key.which);
                break;
            ...
            case 40:
                ...
                playerId.attr("src", mario_down_1);
                playerId.animate({top: "+=20px"}, 10);
                //$('#lastKeyPress').text("Last Key: " + key.which);
                break;
            ...
            case 70:
                ...
                playerId.effect('bounce', {times:1}, 400);
                //$('#lastKeyPress').text("Last Key: " + key.which);
                break;
    ...
    $(document).mousedown(function(event) {
        switch (event.which) {
            ...
            case 1:
                ...
                var bullet_id = $("#" + bullet.id);
                var id = document.getElementById(bullet.id);
                bullet_id.stop().animate({left:event.pageX, top:event.pageY});
                //var a = getDirection(player.xCoord,player.yCoord,xClickCoord,yClickCoord);
                //bullet.movedirection(a);
				/*bullet_id.animate({left: xClickCoord*multiX, top: yClickCoord*multiY}, 1000,  function() {
            ...
            case 3:
                ...
                xClickCoord = event.pageX; //mouse click x coord
                yClickCoord = event.pageY; //mouse click y coord
                a = getDirection(player.xCoord,player.yCoord,xClickCoord,yClickCoord);
    ...
    var menuNotif = $("#notification-box");
    var MessageElement = document.getElementById("notification-box").innerHTML;
    var menuNotifConfPos = $("#notif-confirm-pos");
    var menuNotifConfNeg = $("#notif-confirm-neg");
    ...
    menuItem.hover(function(){
        ...
        $(this).css("background-color", "rgba(255, 117, 26,.7)");
    },function(){

        $(this).css("background-color", "rgba(255, 0, 0, .9)");
    });

	/*menuNotifConfPos.hover(function(){
	 ...
	 menuNotifConfNeg.hover(function(){

	 $(this).css("background-color", "rgba(0, 200, 0,1)");
	 },function(){
    ...
    menuBut.click(function(){
        ...
        else{console.log("ERROR: Failure to determine menu window open.");}
    });
    ...
    menuItem.click(function(){
        ...
        var beef = this.id;
        if(beef === "option1"){ // Resume Game
            menuBox.css("display", "none");
            menuOpen=false;
        }
        else if(beef === "option2"){ // Save Game
            alert("Cannot Save Game At This Time");
        }
        else if(beef === "option3"){ // Load Game
            alert("Cannot Load Game At This Time");
        }
        else if(beef === "option4"){ // Options
            alert("No Options Available At This Time");
        }
        else if(beef === "option5"){ // Restart Game
            document.getElementById("notification-box").innerHTML = '<h3>Are You Sure You Want To Restart The Game?</h3><h3 id="notif-confirm-pos" style="width:25%;float:left;margin-left: 5%;" >Yes</h3><h3 id="notif-confirm-neg" style="width:25%;float:right;margin-right: 5%;" >No</h3>';
            document.getElementById("notif-confirm-pos").onclick = function() {location.reload();};
            document.getElementById("notif-confirm-neg").onclick = function() {menuNotif.css("display", "none");};
...
// Player Object Constructor
    function Player(name) {
        ...
        $(playerId).attr("left", this.xCoord + "px");
        $(playerId).attr("top", this.yCoord + "px");

        //Method to get x,y coordinates of the player
        ...
        //Method to move player in a specified bearing
        this.movedirection = function(bear) {
            var speed = 500;
            var b;
            var sign1,sign2 = "";
            var xValue,yValue;
            b = travelDirection(speed,bear);
            if(b[1] === false){sign1 = "+="}
            ...
            xValue = b[0];
            yValue = b[2];
            console.log("b[0]: "+b[0]+",b[2]"+b[2]);
            $(playerId).animate({left: sign1+xValue+"px", top: sign2+yValue+"px"}, 1000);
        ...
        //Method to move slide player leftwards across the screen
        this.slide = function() {
            $(playerId).animate({left: "-=3px"}, 10);
            this.xCoord = this.xCoord - 3;
            //console.log("(slide)player x: " + this.xCoord);
        };
        //Method to shoot a bullet to where the mouse is when the player clicks
		/*this.shoot = function(e) {
		 ...
		 xBulletCoord = bullet.css('left').split("px");
		 yBulletCoord = bullet.css('top').split("px");
...
// Bullet Object Constructor
    function Bullet(name,x,y) {
        ...
        this.xCoord = x;
        this.yCoord = y;
        $('.bullets').append('<div class="bullet unselectable" id="' + this.id + '"></div>');
        var bulletId = $("#" + this.id);
        bulletId.css({ top: y+'px' });
        bulletId.css({ left: x+'px' });
        ...
        this.movedirection = function(bear) {
            ...
            b = travelDirection(speed,bear,player);
            if(b[1] === false){sign1 = "+="}
            else if (b[1] === true){sign1 = "-="}
            if(b[3] === false){sign2 = "+="}
            else if (b[3] === true){sign2 = "-="}
            ...
            yValue = b[2];
            console.log("b[0]: "+b[0]+",b[2]"+b[2]);
            bulletId.animate({left: sign1+xValue+"px", top: sign2+yValue+"px"}, 1000);
            console.log("bullet complete");
    ...
    function main() {
        ...
        $(document).keydown(function(e) {
            if((e.keyCode != null) && (hasPressed === false)) {
                ...
                //Checks every 30ms to see if the player has gone past the edge of the screen.
...
// P for position, D for destination
    ...
    // Returns a coordinate point at given distance and bearing, traveling from self
    function travelDirection(dist, bear, self){
        //return coordinates and pos/neg signs of a point, given a distance and bearing from a point
        var distance = dist;
        var bearing = bear;
        console.log(self);
        var dx,dy,radian=0;
        var dysign,dxsign = false; // false means not negative
        var array = [];
        var xSelf = self.xCoord;
        var ySelf = self.yCoord;
        //degree = radians * 180/pi

        // Q1
        if((bearing > 0) && (bearing <= 90)){ // Quadrant 1
        ...
        dx = Math.abs(dx);
        dy = Math.abs(dy);
        if(dxsign === false){dx += xSelf;}
        else if (dxsign === true){dx = xSelf - dx;}
        if(dysign === false){dy += ySelf;}
        else if (dysign === true){dy = ySelf - dy;}
...
//Function that moves the background leftwards
    function moveBg() {
        ...
        pos -= 3;
        var speed = 1;
        $(".moving-background").css({backgroundPosition: (pos * speed) + "px 0px"});
